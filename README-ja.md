...
- [pandoc](https://pandoc.org) とローカルのChrome/Chromiumがあれば、Markdownファイルもレンダリング済みプレビューとして開ける——詳しくは後述の[Markdown対応(実験的機能)](#markdown対応実験的機能)を参照
...
- [pandoc](https://pandoc.org) — Markdownをレンダリング済みプレビューとして開くときだけ必要です。なければ `.md` はプレーンテキスト表示にフォールバックします。
- ローカルのChrome/Chromium — Markdownプレビューと、macOS上でのOffice/Keynote/Pagesなど（Quick Look経由）の表示に必要です。Chromeが必要なファイルで使えない場合は警告を出してスキップされます。
- Python 3.9以上
...
Markdownプレビューを使う場合は [pandoc](https://pandoc.org) もインストールします:
...
positional arguments:
  file                  PDF・画像・テキスト・Markdown（.md など）・
                        Quick Lookでプレビュー可能なファイルへのパス（複数可）。
                        1つも指定しない場合（あるいは"-"を指定した場合）は
                        標準入力から読み込む
...
options:
  ...
  -d, --debug           Quick Lookプレビュー生成の各段階（qlmanage、pdftocairo、
                        ...
                        標準エラー出力に表示する
  -s, --rendering-scale N
                        MarkdownおよびQuick Lookプレビュー（Word/Excel/
                        PowerPointなど）をレンダリングする際のデバイス
                        ピクセル比。大きくするとズームイン時に鮮明になるが、
                        レンダリングが遅くなる（デフォルト: 1）
  -c, --continuous      MarkdownまたはQuick Lookプレビューファイルを、
                        常に連続スクロール表示に強制する
...
[pandoc](https://pandoc.org) とローカルのChrome/Chromiumがあれば、`pdfless` は Markdown ファイル（`.md`、`.markdown`、`.mdown`、`.mkd`）も HTML にレンダリングしたプレビューとして開けます。pandoc が standalone HTML に変換し、ヘッドレス Chrome がそれをページ画像にスクリーンショットします（Office プレビューと同じキャプチャ経路ですが、Quick Look や macOS は不要です）。

これは実験的な機能です。デフォルトでは US Letter 相当の高さでページ分割されるため、PDF と同様に `n`/`p` や `<数字><` でページ送りできます。`-c`/`--continuous` を付けると 1 枚の長い連続スクロールになります。Markdown には印刷用のページ境界がないため、分割位置は固定高さでの機械的な切り出しとなり、段落や見出しの途中で切れることがあります。

| 項目 | 内容 |
| --- | --- |
| 拡張子 | `.md`、`.markdown`、`.mdown`、`.mkd` |
| ページ送り | デフォルトは Letter サイズで分割；`-c` で連続表示 |
| テキストモード | ○ — 生の Markdown ソース（`t`）。検索はここでのみ可能（レンダリング画像自体は検索不可） |
| 埋め込み画像 | `.md` ファイルからの相対パスを解決して表示 |

pandoc または Chrome が使えない場合、`.md` は他のテキストファイルと同様にプレーンテキスト表示（ソースそのまま）にフォールバックします。

`-s`/`--rendering-scale` と `-c`/`--continuous` は Markdown プレビューにも適用されます——[使い方](#使い方)を参照してください。
...
| `t` | プレーンテキスト表示に切り替え——PDFならページから抽出したテキスト、`.md` なら生の Markdown ソース、Word系ファイル（`.doc`/`.docx`/`.rtf`など）なら文書全体のテキスト。画像・表計算・スライドには抽出できるテキストがないため何もしない |
