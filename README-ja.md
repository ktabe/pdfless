...
### `pdfless` を `$PAGER` として使う

`file` を1つも指定しない（あるいは代わりに `-` を指定する）と、`pdfless` は標準入力から読み込みます——多くのツールが `$PAGER` を呼び出すときの形そのものです:

```sh
export PAGER=/path/to/pdfless.py
git log     # や man some-command, journalctl など
```

あるいは、環境変数を設定せずその場限りで:

```sh
git log | pdfless.py
```

入力全体を最初に読み切ってからプレーンテキストファイルとして表示するので（`tail -f` のようにリアルタイムに追従するわけではなくスナップショットです）、標準入力自体はパイプであっても、キー入力は実際の端末から受け付けます——`less(1)`/`most(1)` が使っているのと同じ仕組みです。カラー出力するツールの生のANSIエスケープコードは解釈されません——多くのツールは出力先が端末でなければ自動的に色付けをやめますが、`--color=always` などで強制した場合は色ではなく `^[` というノイズとして表示されます。
...
| `E` | （テキストモード）行末に印（↵）を表示するかどうかを切り替え。デフォルトで表示（`-E`/`--no-eol-mark`で最初から非表示にできる）。小文字の`e`は1行進むキーなので大文字 |
...
| `^L` | 画面を再描画 |
| `F1` / `:h` | キーバインドのヘルプを表示（`q` で閉じる）。`less(1)`はこれを`h`/`H`に割り当てているが、どちらも`pdfless`ではパン用のキーであり、`?`も`less(1)`では後方検索のため、`F1`（と、予備の`:h`）としている |
| `q` / `:q` / `^C` | 終了 |
