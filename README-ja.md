...
| `r` | スクロールバー（画像モード・テキストモードどちらでも、端末の右端の1列に現在位置を表示）を表示するかどうかを切り替え。グレーのトラックに、表示中の範囲を示す白いブロック（ファイルが1画面に収まる場合は列全体を占める）。表示のみで、クリックはできない。デフォルトで表示（`--no-scrollbar`で最初から非表示にできる） |
| `^L` | 画面を再描画 |
