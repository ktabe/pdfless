...
- 複数のファイルを開いて切り替え．
- フォローモードでは，表示中のファイルが更新された際に自動で再読み込み．
...
| `-F`, `--follow` | フォローモードで起動します：表示中のファイルの更新を3秒ごとに確認し，ページと表示モードを維持して再読み込みします． |
...
`--follow`で監視するのは，現在表示中のファイルのみです．また，`F`キー（後述）でいつでもON/OFFを切り替えられ，
有効な間はステータス行に`follow`と表示されます．
...
| `F` | フォローモードを切り替えます（`-F`/`--follow`と同じ，デフォルトはOFF）．有効な間はステータス行に`follow`と表示されます． |
