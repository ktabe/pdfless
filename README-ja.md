...
| `r` | スクロールバー（画像モード・テキストモードどちらでも、端末の右端の1列に文書全体に対する現在位置を表示）を表示するかどうかを切り替え。グレーのトラックに、表示中の範囲を示す白いブロック（ファイルが1画面に収まる場合は列全体を占める）。画像モードではクリックでその位置へジャンプでき、そのままドラッグして移動もできる。テキストモードでは端末自身のクリック＆ドラッグによるテキスト選択を残すため表示のみ。デフォルトで表示（`--no-scrollbar`で最初から非表示にできる） |
| `^L` | 画面を再描画 |
