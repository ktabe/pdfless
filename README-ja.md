...
options:
  ...
  -c, --continuous      Quick Lookプレビューファイル（macOSのみ）を、常に
                        連続スクロール表示に強制する——詳細は後述の
                        Office文書対応（実験的機能）を参照
...
| `f` | （テキストモード）ページの外枠に罫線を表示/非表示。デフォルトは表示（`--no-frame` で最初から非表示にできる）。プレーンテキストファイルは、そもそも罫線で囲うような「ページ」がないことが多いので、この指定に関わらず最初から非表示 |
| `^L` | 画面を再描画 |
