...
`less(1)` とほぼ同じキーバインド（行単位/半ページ・1ページ単位のスクロール、ページジャンプなど）で、ターミナル上にそのままPDFを表示できます。ズームイン/アウトもサポートしています。

正規表現で文書全体を検索してマッチ箇所へジャンプすることも、現在のページをいつでも（`t`）プレーンテキスト表示に切り替えることもできます。テキストをコピーしたいときや、レンダリングされた画像ではなくテキストとして読みたいときに便利です。
...
- iTerm2のインラインイメージプロトコルに対応した端末 — [iTerm2](https://iterm2.com) と [WezTerm](https://wezterm.org) で動作確認済み。他の端末でも動く可能性はありますが、このプロトコルに非対応の端末では何も表示されません。
...
ズームとパン（PDFのページはラスタライズするまでスクロール可能な「行」を持たないため、pdfless独自の機能です）:
