...
| `t` | プレーンテキスト表示に切り替え（ページ/行単位でスクロール可能。端末幅より長い行は `h`/`l`/`H`/`L` でパン可能）。PDFなら現在のページから抽出したテキスト、Word系のQuick Lookプレビューファイル（`.doc`/`.docx`/`.odt`など）ならmacOSの`textutil`で抽出した文書全体のテキスト（ページ分割はされず、`n`/`p`はこの表示では無効）。画像ファイル・表計算ファイル・スライドではステータスバーに表示できる旨がないと表示されるだけ（プレーンテキストファイル——`.rtf`も、生のマークアップではなく`textutil`による実際のテキストとして表示される——は最初からこの表示なので切り替え不要） |
