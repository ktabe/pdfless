...
options:
  ...
  -d, --debug           Quick Lookプレビュー生成の各段階（qlmanage、pdftocairo、
                        ...
                        標準エラー出力に表示する
  -s, --rendering-scale N
                        Quick Lookプレビュー（Excel/PowerPoint/Keynoteなど、
                        macOSのみ）をレンダリングする際のデバイスピクセル比。
                        大きくするとズームイン時に鮮明になるが、レンダリング
                        が遅くなる（デフォルト: 1）。Word/RTF/Pagesには効果
                        なし——これらは実PDFとしてレンダリングされ、ズーム
                        しても常にシャープ
...
Word・RTF・Pagesは、内部で(スクリーンショットではなく)Chromeのheadless `--print-to-pdf`を使って実PDFとしてレンダリングされます。そのため、他の形式と違い、通常のPDFと同じようにどのズーム倍率でもシャープなままで、元の文書内のハイパーリンクも本物のPDFと同様にクリック可能です。
