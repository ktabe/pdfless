...
see Viewer._default_text_frame(). The f key must still toggle it
either way."""
...
def make_viewer(path, kind, npages, frame=True):
    _master, slave = pty.openpty()
    fcntl.ioctl(slave, termios.TIOCSWINSZ, struct.pack("HHHH", 40, 120, 960, 720))
    tmpdir = tempfile.mkdtemp()
    viewer = pdfless.Viewer(
        [(os.path.abspath(path), kind, npages)], 0, 1, tmpdir, slave, None, frame=frame,
    )
...
def test_plain_text_file_defaults_to_no_frame_even_without_no_frame_flag(sample_text):
    viewer = make_viewer(sample_text, "text", 1, frame=True)
...
def test_pdf_text_mode_frame_default_unaffected(sample_pdf):
    viewer = make_viewer(sample_pdf, "pdf", pdfless.pdf_page_count(sample_pdf), frame=True)
    assert viewer.enter_text_mode() is True
    assert viewer.text_frame is True  # unchanged - only "text" kind defaults to off

    viewer2 = make_viewer(sample_pdf, "pdf", pdfless.pdf_page_count(sample_pdf), frame=False)
...
def test_office_document_text_mode_frame_default_unaffected(sample_docx):
    viewer = make_viewer(sample_docx, "office", None, frame=True)
    assert viewer.enter_text_mode() is True
